...
function append_child_pages() {
     ...
     */
    $sql = "SELECT * FROM $wpdb->posts WHERE post_parent = " . $post->ID . " AND post_type = 'page' AND post_status = 'publish' ORDER BY menu_order";
...
/* * **Equal Height Child Page Blocks Script*** */

function equal_height_children() {
    ?>
    <script>
        ...
        jQuery('.child-pages > .child > .page_excerpt').each(function() {
...
add_action('pagelines_inside_bottom_postloop', 'equal_height_children', 10);
                    ...
                    <div class="span4">
                        <div class="single-vid-info">
                            ...
                            if ($highlight_text !== '') {
                                ...
                                wp_enqueue_script(
                                        'custom-scroll', get_stylesheet_directory_uri() . '/js/jquery.mCustomScrollbar.concat.min.js', array('jquery')
                                );
                            }
                            ?>
                        </div>
...
function save_featured_auth_field($user_id) {
    ...
    if (!isset($_POST['featured_auth']))
        $_POST['featured_auth'] = "false";
